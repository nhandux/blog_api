                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="table-option d-flex justify-content-between align-items-center pb-2">
                                        ...
                                        <div>
                                            <router-link :to="{ name: 'FormQuestion' }" class="btn btn-primary mt-3 mt-sm-0 ml-0 ml-sm-auto float-right min-w-100 btn-flex"><i class="cil-plus"></i>Register</router-link >
                                            <button @click="deleteQuestion" class="btn btn-danger mt-3 mt-sm-0 ml-0 ml-sm-auto float-right mr-2 min-w-100 btn-flex"><i class="cil-trash"></i>Delete</button>
                                        </div>
                                    </div>
                                    <div class="mb-0">
                                        <div class="table-responsive">
                                            <table class="table table-bordered table-hover table-click-tab">
                                                    ...
                                                    <tr>
                                                        <th class="text-center" width="90px">
                                                            <div class="custom-control custom-checkbox d-flex justify-content-center">
                                                                <input id="checkedAll" :disabled="question.data && question.data.length <= 0" @click="selectAllViaCheckBox" v-model="select_all" type="checkbox" class="custom-control-input change-status" autocomplete="off">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr v-for="item in question.data" :key="item.id" >
                                                        ...
                                                        <td align="center">
                                                            <router-link  class="btn btn-primary btn-small" :to="{ name: 'UpdateQuestion', params: { id: item.id }}"><i class="cil-pencil"></i></router-link>
                                                        </td>
                                                    </tr>
                                                    <tr v-if="question.data && question.data.length===0"><td class="text-center" colspan="6">Data empty, comming soon....</td></tr>
                                                </tbody>
                                            </table>
                                        </div>
                                        <pagination v-if="question.total"
                                            :total-pages="question.last_page"
...
<script>
    import { index, questionDelete } from '../../../helpers/question';
    ...
    export default {
        ...
        created: function(){
            this.getListQuestion(this.search);
        },
        methods: {
            getListQuestion(search){ 
                this.$store.commit("LOAD_QUESTION");
                index(search)
                    .then(res => {
                        this.$store.commit("QUESTION_LIST", res);
                    })
                    .catch(error => {
                        this.$store.commit("LOAD_QUESTION_FAIL");
            ...
            selectAllViaCheckBox() {
                if(this.select_all == false){
                    ...
                    this.question.data.forEach(question => {
                      this.items.push(question.id)
                    });
                }else{
            ...
            deleteQuestion() {
                if(this.items.length <= 0) {
                    ...
                    this.$store.commit("LOAD_QUESTION");
                    questionDelete(this.items)
                        .then(res => {
                            this.select_all = false;
                            this.items = [];
                            this.getListQuestion(this.search);
                            this.$toast.success('Delete question successfully!');
                        })
                        .catch(error => {
                            this.$store.commit("LOAD_QUESTION_FAIL");
                            this.$toast.error(error);
        ...
        computed: {
            ...
            question () {
                return this.$store.getters.QUESTION_LIST
            }
        }
    };
</script>
